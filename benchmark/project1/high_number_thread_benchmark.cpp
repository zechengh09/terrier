...
class HighNumberThreadBenchmark : public benchmark::Fixture {
 public:
  ...
  const uint32_t num_concurrent_txns_ = 100;

 private:
  std::thread gc_thread_;
  storage::GarbageCollector *gc_ = nullptr;
  volatile bool run_gc_ = false;
  const std::chrono::microseconds gc_period_{1};
 ...
 */
// NOLINTNEXTLINE
BENCHMARK_DEFINE_F(HighNumberThreadBenchmark, SingleStatementSelect)(benchmark::State &state) {
...
BENCHMARK_REGISTER_F(HighNumberThreadBenchmark, SingleStatementSelect)
